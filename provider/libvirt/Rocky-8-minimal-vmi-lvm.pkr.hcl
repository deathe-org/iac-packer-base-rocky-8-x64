...
build {
  ...
  post-processor "shell-local" {
    ...
    inline = [
      ...
      "  mv -f output-$${PACKER_BUILD_NAME}/${var.guest_name} $${1}/${var.build_name}.$${2}",
      "  printf -- '--> Convert build artifact from %s to %s.\n' $${2} $${3}",
      "  if [[ $${3} == vmdk ]]",
      "  then",
      "    qemu-img convert -f $${2} -O $${3} -o subformat=streamOptimized,compat6 $${1}/${var.build_name}.$${2} $${1}/${var.build_name}.$${3}",
      "  else",
      "    qemu-img convert -f $${2} -O $${3} $${1}/${var.build_name}.$${2} $${1}/${var.build_name}.$${3}",
      "  fi",
      "fi",
