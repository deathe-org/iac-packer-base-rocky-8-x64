...
source "virtualbox-iso" "build" {
  ...
  vboxmanage              = [["modifyvm", "{{.Name}}", "--accelerate2dvideo", "off"], ["modifyvm", "{{.Name}}", "--accelerate3d", "off"], ["modifyvm", "{{.Name}}", "--audio", "none"], ["modifyvm", "{{.Name}}", "--biosbootmenu", "disabled"], ["modifyvm", "{{.Name}}", "--bioslogodisplaytime", "0"], ["modifyvm", "{{.Name}}", "--bioslogofadein", "off"], ["modifyvm", "{{.Name}}", "--bioslogofadeout", "off"], ["modifyvm", "{{.Name}}", "--clipboard", "disabled"], ["modifyvm", "{{.Name}}", "--cpus", "1"], ["modifyvm", "{{.Name}}", "--draganddrop", "disabled"], ["modifyvm", "{{.Name}}", "--graphicscontroller", "vmsvga"], ["modifyvm", "{{.Name}}", "--hpet", "on"], ["modifyvm", "{{.Name}}", "--memory", "1024"], ["modifyvm", "{{.Name}}", "--nat-localhostreachable1", "on"], ["modifyvm", "{{.Name}}", "--usb", "off"], ["modifyvm", "{{.Name}}", "--vram", "10"]]
  vboxmanage_post         = [["modifyvm", "{{.Name}}", "--cpus", "${var.guest_cpus}"], ["modifyvm", "{{.Name}}", "--memory", "${var.guest_memory}"], ["modifyvm", "{{.Name}}", "--vram", "${var.guest_vram}"], ["modifyvm", "{{.Name}}", "--vrde", "off"], ["storagectl", "{{.Name}}", "--name", "IDE Controller", "--remove"], ["storagectl", "{{.Name}}", "--name", "SATA Controller", "--hostiocache", "off"]]
...
build {
  ...
  post-processor "shell-local" {
    ...
    inline          = ["if [[ -n $${1} ]] && [[ -n $${2} ]] && [[ -n $${PACKER_BUILD_NAME} ]] && [[ -d output-$${PACKER_BUILD_NAME} ]]; then", "  echo '--> Moving build artifact.'", "  mv -f output-$${PACKER_BUILD_NAME}/${var.guest_name}.$${2} $${1}/$${PACKER_BUILD_NAME}.$${2}", "fi", "echo '--> Cleanup output directories.'", "find . -mindepth 1 -maxdepth 1 -type d -name \"output-*\" -exec rm -rf '{}' +"]
