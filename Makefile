...
with Packer. The AMI variants can also be used to build cloud images
...
Variables (default value):
  ...
  - BOX_VARIANT (minimal)   Used to specify box build variants. i.e.
                              ...
                              - minimal-lvm
                              - minimal-cloud-init
                              - minimal-cloud-init-lvm
                              - minimal-vmi
                              - minimal-vmi-lvm
  - BOX_VERSION_RELEASE     The Rocky-8 Minor Release number. Note: A
    (8.8)                   corresponding template is required.
...
download-iso: _prerequisites _require-supported-architecture
	@ if [[ ! -f isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME) ]]; then \
		...
		echo "$(PREFIX_STEP)Downloading ISO: https://dl.rockylinux.org/pub/rocky/$(BOX_VERSION_RELEASE)/isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME)"; \
		$(curl) \
			--fail \
			--location \
			--silent \
			--output ./isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME) \
			https://dl.rockylinux.org/pub/rocky/$(BOX_VERSION_RELEASE)/isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME); \
		if [[ $${?} -ne 0 ]]; then \
			rm -f ./isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME) &> /dev/null; \
			echo "$(PREFIX_STEP)Download failed - trying vault: https://dl.rockylinux.org/vault/rocky/$(BOX_VERSION_RELEASE)/isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME)"; \
			$(curl) \
				--fail \
				--location \
				--silent \
				--output ./isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME) \
				https://dl.rockylinux.org/vault/rocky/$(BOX_VERSION_RELEASE)/isos/$(BOX_ARCH)/$(SOURCE_ISO_NAME); \
		fi; \
		if [[ $${?} -ne 0 ]]; then \
